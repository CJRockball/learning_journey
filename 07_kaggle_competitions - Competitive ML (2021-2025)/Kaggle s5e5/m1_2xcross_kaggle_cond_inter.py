...
for d in [df_train, df_test]:
    ...
    for dur in d['Duration'].unique():
        d[f'Heart_Rate_Duration_{int(dur)}'] = np.where(d['Duration'] == dur, d['Heart_Rate'], 0)
        d[f'Body_Temp_Duration_{int(dur)}'] = np.where(d['Duration'] == dur, d['Body_Temp'], 0)
    for age in d['Age'].unique():
        d[f'Heart_Rate_Age_{int(dur)}'] = np.where(d['Age'] == dur, d['Heart_Rate'], 0)
        d[f'Body_Temp_Age_{int(dur)}'] = np.where(d['Age'] == dur, d['Body_Temp'], 0)        
