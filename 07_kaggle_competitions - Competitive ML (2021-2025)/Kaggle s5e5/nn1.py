...
def valid(model, loader, criterion, DEVICE):
    ...
    return validation_loss, validation_rmsle 
...
#%%
# DEF MODEL
model = Model(meta_data, 0.1, [256, 128, 64], 3*[0.1]).to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    validation_loss, validation_rmsle = valid(model, validloader, criterion, DEVICE) #, oof, val_idx)
    print(f'Epoch: {epoch}/{EPOCHS}, Train loss: {train_data:.6f}, Validation loss: {validation_loss:.6f}, Validation rmsle: {validation_rmsle:.6f}')
...
validation_loss, validation_rmsle = valid(model, validloader, criterion, DEVICE)
...
trans_pred = np.expm1(y_pred)

df_sub = pd.read_csv('data/ra/sample_submission.csv')
df_sub[target] = trans_pred

df_sub.to_csv('submissions/nn1.csv', index=False)

df_check = pd.read_csv('submissions/nn1.csv')
display(df_check)
